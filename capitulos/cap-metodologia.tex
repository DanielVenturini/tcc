...
Since the customer recovered from the error, there are two ways to know how it recoverer from the \textit{breaking change}. The first way is when the provider fixes her code and the client just updates the string of versioning in \textit{package.json}, if it needs. To the provider fixes the error, one issue may be done in her repository. The second way is when the client must do some work to fix the code. In this case, the client can fix the provider code and do a pull-request or change her code to work with the provider. And of course, may have cases that anyone does nothing. There is a breaking change and it’s never been fixed.

The general way to get information about this RQ is in the \textit{GitHub}. This information can be found at \textit{CHANGELOG, release-notes, issues,} and \textit{pull-requests}. If the changelog contains the information, in general, the \textit{issues} are marked. From the \textit{issue}, a lot of more information can be recovered, like \textit{pull-requests} that are also marked in the \textit{issue}, other \textit{issues}, commentaries and more. All of this information can help us to discover which one -- client or provider -- fixed the \textit{breaking change} and how fixed.

To verify in the client repository, the \textit{commits} are the alternative ways to \textit{issues}. The \textit{commits} contain all updated providers in \textit{package.json}. Commits message like \textit{update dependencies, fix dependencies, fix errors} an so on, suggests that something about any dependencies was fixed. This information is very important, because, since the provider was fixed and the client just updates it, the commit messages can tell the reason for this update - or downgrade. And then, we discovered which one and how the breaking change was fixed.
