...
\label{fin:rq1}
Ao todo, 384 pacotes foram utilizados nesta pesquisa e foram executadas pelo menos uma de suas \textit{releases}. Desses 384 pacotes, 244 resultaram em falhas para alguma de suas \textit{releases}. Já analisando as \textit{releases}, de todas as 4570, foram executadas um total de 2662 \textit{releases}, pois possuíam adições ou alterações em suas dependências e estavam passíveis de sofrerem com \textit{breaking change}. Em contra partida, 1908 \textit{releases} não foram executadas pois não haviam alterações nas \textit{releases} dos provedores, mas apenas alterações no código do cliente, o que não tem possibilidade da manifestação de \textit{breaking changes}. Após a execução, 1460 \textit{releases} resultaram em erro no \textit{script install/test}, e foram analisadas manualmente. A Tabela \ref{tab:res_rq1} contém os resultados prévios sem a análise manual.
...
Após executarem, todas as \textit{releases} que resultaram em erro foram analisadas manualmente para separar os erros falso-positivos, erros internos e \textit{breaking changes}.
