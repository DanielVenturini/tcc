...
Uma \textit{break change} é uma alteração no pacote provedor que resulta em erros inesperados nos pacotes clientes, uma vez que versões prévias deste pacote provedor executava normalmente \cite{teorical_reference:semver}.

As \textit{breaking changes} são mais expressivas em linguagem fortemente tipadas, tais como \textit{.NET} e \textit{Java}, tanto que há ferramentas para detectar \textit{breaking changes} nestas linguagens. Já nas linguagens mais dinâmicas, como a \textit{Javascript}, as \textit{breaking changes} surgem com menos frequências, e nem possuem ferramentas para detectar as \textit{breaking changes} \cite{teorical_reference:bc_1}. Como exemplo da dinâmica do \textit{Javascript}, considere o Código \ref{cod:non_bc_js}.
% As breaking changes levam a melhoria do projeto mas podem ser adicionadas em releases path ou minor

\begin{lstlisting}[style=javascript, label=cod:non_bc_js, caption={Acesso inválido à posição de memória}, numbers=none]
// funcao foo na versão 1.0.0 de uma biblioteca
foo = function(var1, var2, var3) { /* ... */ }

// funcao foo na versão 1.1.0 de uma biblioteca
foo = function(var1) { /* ... */ }
...
% no semantic version vs breaking change, o java possui como terceira maior bc a remoção de parametros, enquanto no Javascript isto não ocorre