...
\daniel{Todas as vezes eu me refiro 'pacote cliente', 'pacote provedor'. Não seria melhor só 'cliente' e 'provedor'? O que o senhor sugerir eu vou consertar no texto todo}
...
\daniel{Vou alterar essa figura e colocar naquele esquema de duas figuras dentro de uma direto no latex. Pera, acho que essa figura de cima com o stack tracer não precisa. O que o senhor acha?}
\begin{figure}
    \centering
...
\begin{itemize}
    ...
    \daniel{O filipe e wiese falaram para não considerar a exclusão como uma breaking change, mas apenas para contabilizar como uma non-breaking change. Então eu mudo essa descrição? pois eu falo que o erro é causado pelo node.js. Devo falar que o erro é causado pelo provedor mas não tem condição de ser analisado?}
