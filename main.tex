\documentclass[12pt,english,brazil,a4paper,utf8,oneside]{utfpr-tcc}

%Reviewing
\newcommand{\filipe}[1]{\textcolor{blue}{{\it [Filipe: #1]}}}
% copying from you, Filipe, hahaa
\newcommand{\daniel}[1]{\textcolor{red}{{\it [Daniel: #1]}}}

% carrega o arquivo configuracoes.tex que contém os pacotes e comandos Latex.
\include{configuracoes}
% carrega o arquivo constantes.tex que contém dados do curso/monografia que NÃO DEVEM ser alterados. 
\include{constantes}
% carrega o arquivo variaveis.tex que contém dados do acadêmico/monografia que DEVEM ser alterados.
\include{variaveis}

% carrega o arquivo listaabreviaturas.tex que está dentro do diretório pretextual, esse arquivo contém as siglas utilizadas na monografia.
\include{pretextual/listaabreviaturas} % usando glossaries

\begin{document}

\frontmatter
\maketitle

% Dedicatória é opcional, para usar descomentar a linha a seguir e edite o arquivo pretextual/dedicatoria.tex
%\dedicate{\input{pretextual/dedicatoria}} % Opcional - descomentar para usar

% Agradecimento é opcional, para usar descomentar a linha a seguir e edite o arquivo pretextual/dedicatoria.tex
%\include{pretextual/agradecimentos} % Opcional - descomentar para usar

% carrega o arquivo resumo.tex que está dentro do diretório pretextual, esse arquivo deve conter o resumo da monografia.
\include{pretextual/resumo}
\include{pretextual/abstract}
% carrega o arquivo abstract.tex que está dentro do diretório pretextual, esse arquivo deve conter um resumo escrito na linguá inglesa para a monografia.
%\include{pretextual/abstract}

% Listas (opcionais, mas recomenda-se a partir de 5 elementos)
\listoffigures
\listoftables
%\listofacronyms
\printnoidxglossaries

% Sumário
\tableofcontents

\mainmatter

% Capítulos da monografia:
\include{capitulos/cap-introducao}
\include{capitulos/cap-ref-teorico} % Esse capítulo e nome é apenas uma sugestão.
\include{capitulos/cap-exemplos} % Esse capítulo e nome é apenas uma sugestão.
\include{capitulos/cap-questoes} % Esse capítulo e nome é apenas uma sugestão.
\include{capitulos/cap-metodologia} % Esse capítulo e nome é apenas uma sugestão.
% \include{capitulos/cap-res_pre} % Esse capítulo e nome é apenas uma sugestão.
\include{capitulos/cap-resultados} % Esse capítulo e nome é apenas uma sugestão.
% \include{capitulos/cap-res-rq1} % RQ1 está dentro do "capitulos/cap-resultados"
\include{capitulos/cap-res-rq2}
\include{capitulos/cap-res-rq3}
\include{capitulos/cap-discussoes} % Esse capítulo e nome é apenas uma sugestão.
\include{capitulos/cap-ameacas} % Esse capítulo e nome é apenas uma sugestão.
\include{capitulos/cap-conclusoes} % Esse capítulo e nome é apenas uma sugestão.

% Apendices.
%\appendix
%\include{postextual/ape-instalacao-ferramentas}

%bibliografia
\bibliographystyle{abntex2-alf}
\bibliography{main} % geração automática das referências a partir do arquivo main.bib

\backmatter
\end{document}
